...
dji = pd.read_csv("/home/prahlad/myenv/lib/python3.7/site-packages/gym/envs/zxstock/Data_Daily_Stock_Dow_Jones_30/^DJI.csv")
...
class StockTestEnv(gym.Env):
    ...
    def __init__(self, day = 0, money = 10 , scope = 1):
        ...
        self.action_space = spaces.Box(low = -5, high = 5,shape = (1,),dtype=np.int8) 
        ...
        self.state = [10000] + self.data.adjcp.values.tolist() + [0]
        self.reward = 0
    ...
    def _sell_stock(self, index, action):
        if self.state[index+2] > 0:
            self.state[0] += self.state[index+1]*min(abs(action), self.state[index+2])
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            plt.savefig('/home/prahlad/test_stock_VISA.png')
            plt.close()
            print("total_reward:{}".format(self.state[0]+ self.state[1]*self.state[2] - 10000 ))
            ...
            self.state =  [self.state[0]] + self.data.adjcp.values.tolist() + [self.state[2]]
